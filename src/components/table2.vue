...
export default{
    setup(){
        ...
        //convert all of our computed functions from vue 2 to vue 3 following 
        ...
        function hideShowALLstatus() {
            state.allCheck = !state.allCheck; //toggle the check
            if (state.allCheck) { //hidestatus to contain all statuses
                state.hidestatus = state.productDataBystatus.status;
                state.allCheckBox = state.productDataBystatus.status;
            } else { //else clear the arrays
                state.hidestatus = [];
                state.allCheckBox = [];
            }
        }
        onMounted(() => {
            state.wwInfo = getWWFromDate();
        });
